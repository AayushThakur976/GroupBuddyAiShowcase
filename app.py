...
def process_in_background(user_message, session_id):
    """
    This function runs the long-running graph invocation in a separate thread
    and posts the final response back to the chat via webhook.
    """
    # This 'with' block gives the background thread the context it needs
    with app.app_context():
        print(f"🤖 Starting background processing for session: {session_id}")
        try:
            # The "typing..." message has been removed for a cleaner experience.
